...
module register_set #(parameter MSB=40)   (   // 1 byte address, and write flag,   4 bytes data.
  ...
  */
  ...
  reg [32-1:0]  out  ;    // register for output.  should be size of MSB due to high bits
  reg [8-1:0]   count;
        ...
        if(count == 8)    // TODO - THINK WE SHOULDUSE bcount==8 here.  and this might fix the top bit issue.
          begin
            ...
            case (in[8 - 2  : 0 ] )
              ...
              `REG_TEST1:                         out <= 32'b00001111000011110000111100001111 << 8;
              `REG_TEST2:                         out <= 32'b11110000111100001111000011110000 << 8;
